...
class LocalAttention(nn.Module):
    ...
    def forward(self, matrix, position):
        ...
        x, y = position
        local_Q = Q[x - self.window_size : x + self.window_size + 1, y - self.window_size : y + self.window_size + 1]
        local_K = K[x - self.window_size : x + self.window_size + 1, y - self.window_size : y + self.window_size + 1]
        local_V = V[x - self.window_size : x + self.window_size + 1, y - self.window_size : y + self.window_size + 1]

        attention = torch.matmul(local_Q, local_K.transpose(-2, -1)) / (self.embed_size ** 0.5)
...
class NonLinearTransformer(nn.Module):
    def __init__(self, vocab_size, embed_size, matrix_dim, heads, window_size, iterations):
